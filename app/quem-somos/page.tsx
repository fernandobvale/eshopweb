...
import { buildAboutUsHtml } from "@/lib/legal-pages";
...
export async function generateMetadata(): Promise<Metadata> {
    ...
    const title = `Quem Somos | ${siteName}`;
    const description = `Conheça mais sobre o ${siteName}.`;
...
export default async function AboutUsPage() {
    ...
    const [settingsRes, categoriesRes, pageRes] = await Promise.all([
        ...
        supabase.from("pages").select("*").eq("slug", "quem-somos").eq("status", "published").maybeSingle(),
    ]);

    const settings = settingsRes.data as SiteSettings | null;
    const categories = (categoriesRes.data ?? []) as Category[];
    const siteName = settings?.display_title || settings?.site_name || "SEO Blog";
    const defaultContent = buildAboutUsHtml({
    ...
    const webPageJsonLd = buildWebPageJsonLd({
        ...
        description: `Conheça mais sobre o ${siteName}.`,
        url: "/quem-somos",
    });
