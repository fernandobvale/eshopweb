...
import { buildPrivacyPolicyHtml } from "@/lib/legal-pages";
...
export async function generateMetadata(): Promise<Metadata> {
    ...
    const title = `Política de Privacidade | ${siteName}`;
    const description = `Leia a Política de Privacidade do ${siteName}.`;
...
export default async function PrivacyPolicyPage() {
    ...
    const [settingsRes, categoriesRes, pageRes] = await Promise.all([
        ...
        supabase.from("pages").select("*").eq("slug", "privacidade").eq("status", "published").maybeSingle(),
    ]);

    const settings = settingsRes.data as SiteSettings | null;
    const categories = (categoriesRes.data ?? []) as Category[];
    const siteName = settings?.display_title || settings?.site_name || "SEO Blog";
    const defaultContent = buildPrivacyPolicyHtml({
    ...
    const webPageJsonLd = buildWebPageJsonLd({
        ...
        description: `Leia a Política de Privacidade do ${siteName}.`,
        url: "/privacidade",
    });
